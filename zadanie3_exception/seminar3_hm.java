package geekbrains_course.exception_course;
...
public class seminar3_hm {
	...
	public static void main(String[] args) {
		...
		if (fields.length != fields_number) {
			System.out.println("Wrong number of fields, you need 6 but wrote " + fields.length);
		}
		...
		if ( (!"m".equals(gender) ) && (!"f".equals(gender) ) ) {
			System.err.println("wrong gender format, write f or m");
			return;
		}
		...
		try (BufferedWriter writer = new BufferedWriter (new FileWriter (fileName, true) ) ) {
			writer.write(lastName + " " + firstName + " " + 
						middleName + " " + birthDate.format(DateTimeFormatter.ISO_LOCAL_DATE) 
						+ " " + phoneNumber + " " + gender);
			writer.newLine();
		}
		catch (IOException e) {
